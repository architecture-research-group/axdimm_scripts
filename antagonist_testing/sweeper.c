...
int main(int argc, char ** argv)
{
	...
	while ( 1 ){
		offset+=SIZE;
		char * src = (char *) mmap(NULL, SIZE, PROT_READ | PROT_WRITE, MAP_FILE | MAP_SHARED, cdevfd, base+offset );
		if (src == -1)
		{
			printf("could not SRC MEM Char Dev\n");
			exit(-1);
		}
		char * dst = (char *) mmap(NULL, SIZE, PROT_READ | PROT_WRITE, MAP_FILE | MAP_SHARED, cdevfd, base + dst_off +offset);
		if (dst == -1)
		{
